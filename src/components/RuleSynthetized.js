...
export default function RuleSynthetized() {
  ...
  return (
    <div className="flex-wrap border-2 rounded-lg shadow-lg w-96 m-5 p-5 text-lg">
      ...
      {
        ((rule.rule[action] || {}).constraints || []).map((andConstraints, index) => {
          let stringRule = "" + (index + 1) + ". "
          for (const [index, constraint] of andConstraints.entries()) {
            if (index > 0) {
              stringRule += " and "
            }
            stringRule += constraint.state + " " + constraint.operator + " " + constraint.value.toFixed(2)
          }
          return (
            <p key={index}>{stringRule}</p>
          )
        })
      }
